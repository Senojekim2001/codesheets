#!/usr/bin/env python3
"""
Audit script for Python vault entries.
Checks for:
1. Tier consistency (beginner/junior/senior code solving same problem)
2. Structural errors and formatting issues
3. Missing sections or malformed content
"""
...
class VaultAuditor:
    ...
    def audit_entry(self, entry_path: Path) -> List[str]:
        ...
        if has_intro and has_junior and has_senior:
            ...
            # Log missing tiers with file path for debugging
            if not has_intro:
                self.stats['missing_intro'] += 1
                issues.append("Missing intro tier")
                print(f"DEBUG: Missing intro in {entry_path.relative_to(self.vault_path)}")
            if not has_junior:
                self.stats['missing_junior'] += 1
                issues.append("Missing junior tier")
                print(f"DEBUG: Missing junior in {entry_path.relative_to(self.vault_path)}")
            if not has_senior:
                self.stats['missing_senior'] += 1
                issues.append("Missing senior tier")
                print(f"DEBUG: Missing senior in {entry_path.relative_to(self.vault_path)}")
    ...
    def print_report(self, results: Dict[str, any]):
        ...
        print("\n" + "="*80)
        print("PYTHON VAULT AUDIT REPORT")
        print("="*80)
